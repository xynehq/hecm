...
class ClaudeProxyDatasetGenerator:
    ...
    def _generate_claude_attempt(
        ...
        try:
            ...
            print("Claude Patch:\n", claude_patch)
            self.logger.info(f"Claude Patch for {data_point.instance_id}:\n{claude_patch}")
            files_changed = self._get_changed_files(test_repo)
            self.logger.info(f"Files changed for {data_point.instance_id}: {files_changed}")
            print("Files Changed:\n", files_changed)
        ...
        try:
            ...
            if max_workers == 1:
                ...
                for data_point in tqdm(data_points, desc="Generating Claude attempts"):
                    attempt = self._generate_claude_attempt(data_point, timeout_per_task)
                    if attempt:
            ...
            result_dataset = ClaudeAttemptDataset(
                ...
                dataset_metadata={
                    ...
                    "num_successful": sum(1 for a in claude_attempts if a.claude_success),
                    "num_failed": sum(1 for a in claude_attempts if not a.claude_success),
            ...
            self.logger.info(f"Successful: {result_dataset.dataset_metadata['num_successful']}")
    ...
    def cleanup_temp_directories(self):
        ...
        if self.proxy_repo_path.exists() and self.proxy_repo_path.parent == Path(tempfile.gettempdir()):
